...
public class OfficerUserUpdateActivity extends BaseActivity {
    ...
    private static final String TAG = "OfficerUserUpdate";
    ...
    private void performUpdate(String phone, String email, String cccd, String dob) {
        ...
        boolean cccdChanged = !cccd.equals(originalCccd != null ? originalCccd : "");
        boolean emailChanged = !email.equals(originalEmail);
        boolean dobChanged = !dob.equals(originalDob != null ? originalDob : "");
        
        // Start the update chain
        if (phoneChanged) {
            callUpdatePhoneApi(phone, email, cccd, dob, cccdChanged, emailChanged, dobChanged);
        } else if (cccdChanged) {
            callUpdateCccdApi(cccd, email, dob, emailChanged, dobChanged);
        } else if (emailChanged || dobChanged) {
            callUpdateUserApi(email, dob);
        } else {
            // No changes, just refresh and finish
            refreshUserData();
        }
    }
     ...
     */
    private void callUpdatePhoneApi(String phone, String email, String cccd, String dob,
                                     boolean cccdChanged, boolean emailChanged, boolean dobChanged) {
        UserApiService userApiService = ApiClient.getUserApiService();
        UpdatePhoneRequest request = new UpdatePhoneRequest(phone);
        
        Call<Map<String, Object>> call = userApiService.updatePhoneNumber(userId, request);
        call.enqueue(new Callback<Map<String, Object>>() {
            ...
            public void onResponse(Call<Map<String, Object>> call, Response<Map<String, Object>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (success != null && success) {
                        Log.d(TAG, "Update phone success: userId=" + userId);
                        
                        // Continue with next update if needed
                        if (cccdChanged) {
                            callUpdateCccdApi(cccd, email, dob, emailChanged, dobChanged);
                        } else if (emailChanged || dobChanged) {
                            callUpdateUserApi(email, dob);
                        ...
                        Toast.makeText(OfficerUserUpdateActivity.this, 
                            message != null ? message : "Lỗi cập nhật số điện thoại", 
                            Toast.LENGTH_SHORT).show();
                    }
                } else {
                    showLoading(false);
                    String errorMsg = "Lỗi cập nhật số điện thoại: " + response.code();
                    if (response.code() == 409) {
                        errorMsg = "Số điện thoại đã được sử dụng bởi người dùng khác";
                    }
                    Toast.makeText(OfficerUserUpdateActivity.this, errorMsg, Toast.LENGTH_SHORT).show();
                    Log.e(TAG, "Update phone failed: " + response.code());
                }
            }
            ...
            public void onFailure(Call<Map<String, Object>> call, Throwable t) {
                ...
                Log.e(TAG, "Update phone API call failed: " + t.getMessage(), t);
            }
        });
    }
    
    /**
     * Gọi API cập nhật số CCCD
     * Endpoint: PATCH /api/users/{userId}/cccd
     */
    private void callUpdateCccdApi(String cccd, String email, String dob,
                                    boolean emailChanged, boolean dobChanged) {
        UserApiService userApiService = ApiClient.getUserApiService();
        UpdateCccdRequest request = new UpdateCccdRequest(cccd);
        
        Call<Map<String, Object>> call = userApiService.updateCccdNumber(userId, request);
        call.enqueue(new Callback<Map<String, Object>>() {
            @Override
            public void onResponse(Call<Map<String, Object>> call, Response<Map<String, Object>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (success != null && success) {
                        Log.d(TAG, "Update CCCD success: userId=" + userId);
                        ...
                        String message = (String) body.get("message");
                        Toast.makeText(OfficerUserUpdateActivity.this, 
                            message != null ? message : "Lỗi cập nhật số CCCD", 
                            Toast.LENGTH_SHORT).show();
                    }
                } else {
                    showLoading(false);
                    String errorMsg = "Lỗi cập nhật số CCCD: " + response.code();
                    if (response.code() == 409) {
                        errorMsg = "Số CCCD đã được sử dụng bởi người dùng khác";
                    }
                    Toast.makeText(OfficerUserUpdateActivity.this, errorMsg, Toast.LENGTH_SHORT).show();
                    Log.e(TAG, "Update CCCD failed: " + response.code());
                }
            }
            ...
            public void onFailure(Call<Map<String, Object>> call, Throwable t) {
                ...
                Toast.makeText(OfficerUserUpdateActivity.this, 
                    "Không thể kết nối server: " + t.getMessage(), Toast.LENGTH_SHORT).show();
                Log.e(TAG, "Update CCCD API call failed: " + t.getMessage(), t);
            }
        });
    }
    
    /**
     * Gọi API cập nhật thông tin user (email, dateOfBirth)
     * Endpoint: PUT /api/users/{userId}
     */
    private void callUpdateUserApi(String email, String dob) {
        UserApiService userApiService = ApiClient.getUserApiService();
        
        // Tạo request với các field được phép cập nhật
        ...
        Call<UserResponse> call = userApiService.updateUser(userId, request);
        call.enqueue(new Callback<UserResponse>() {
            @Override
            public void onResponse(Call<UserResponse> call, Response<UserResponse> response) {
                if (response.isSuccessful() && response.body() != null) {
                    Log.d(TAG, "Update user success: userId=" + userId);
                    // Refresh user data after successful update
                    refreshUserData();
                } else {
                    showLoading(false);
                    String errorMsg = "Lỗi cập nhật: " + response.code();
                    if (response.code() == 403) {
                        errorMsg = "Bạn không có quyền thực hiện thao tác này";
                    } else if (response.code() == 404) {
                        errorMsg = "Không tìm thấy người dùng";
                    } else if (response.code() == 409) {
                        errorMsg = "Email đã được sử dụng bởi người dùng khác";
                    } else if (response.code() == 400) {
                        errorMsg = "Dữ liệu không hợp lệ";
                    }
                    Toast.makeText(OfficerUserUpdateActivity.this, errorMsg, Toast.LENGTH_SHORT).show();
                    Log.e(TAG, "Update user failed: " + response.code() + " - " + response.message());
                }
            }
            ...
            public void onFailure(Call<UserResponse> call, Throwable t) {
                ...
                Toast.makeText(OfficerUserUpdateActivity.this, 
                    "Không thể kết nối server: " + t.getMessage(), Toast.LENGTH_SHORT).show();
                Log.e(TAG, "Update user API call failed: " + t.getMessage(), t);
            }
        });
    }
    
    /**
     * Gọi API lấy thông tin user mới nhất sau khi cập nhật
        ...
        call.enqueue(new Callback<UserResponse>() {
            ...
            public void onResponse(Call<UserResponse> call, Response<UserResponse> response) {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    Log.d(TAG, "Refresh user data success: userId=" + userId);
                    ...
                    // Even if refresh fails, the updates were successful
                    Toast.makeText(OfficerUserUpdateActivity.this, 
                        "Cập nhật thành công! Vui lòng tải lại để xem thông tin mới.", 
                        Toast.LENGTH_SHORT).show();
                    setResult(RESULT_OK);
                    finish();
                }
            ...
            public void onFailure(Call<UserResponse> call, Throwable t) {
                ...
                Toast.makeText(OfficerUserUpdateActivity.this, 
                    "Cập nhật thành công! Vui lòng tải lại để xem thông tin mới.", 
                    Toast.LENGTH_SHORT).show();
                Log.e(TAG, "Refresh user data failed: " + t.getMessage(), t);
                setResult(RESULT_OK);
                finish();
            }
        });
    ...
    private void showLoading(boolean show) {
        ...
        btnConfirmUpdate.setEnabled(!show);
    }
}
